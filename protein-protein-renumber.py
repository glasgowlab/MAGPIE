...
def chain_search(chains : str, parsed_pdb : list = None,input_pdb_path: str = None ) -> list:
    ...
    for line in parsed_pdb:
        ...
        elif "ATOM" not in line[0:6]:
            chain_filtered_pdb.append(line)

    return chain_filtered_pdb

def seq_search(seq : str, parsed_pdb : list = None,input_pdb_path: str = None) -> list:

    if input_pdb_path is not None:
        parsed_pdb = parse_pdb(input_pdb_path)

    seq = arg_spliter(seq)
    pdb_seq_hits = find_simular_chains(seq, parsed_pdb, 95)

    return pdb_seq_hits

def seq_and_chain_search(input_pdb_path: str, binder_seq : str = None, antigen_seq : str = None, binder_chains : str = None, antigen_chains : str = None, order : str = "chain") -> dict:
    pdb_hits = {"binder":[],"antigen":[]}

    if (binder_seq and binder_chains):

        if order == "seq":
            pdb_binder_seq_hits = seq_search(binder_seq, None, input_pdb_path)
            ...
            pdb_hits["binder"] = seq_search(binder_seq, pdb_binder_chain_hits, None)

    elif binder_seq:
        pdb_hits["binder"] = seq_search(binder_seq,None,input_pdb_path)
    elif binder_chains:
    ...
    if (antigen_seq and antigen_chains):
        if order == "seq":
            pdb_antigen_seq_hits = seq_search(antigen_seq, None, input_pdb_path)
            ...
            pdb_hits["antigen"] = seq_search(antigen_seq, pdb_antigen_chain_hits, None)
    elif antigen_seq:
        pdb_hits["antigen"] = seq_search(antigen_seq,None,input_pdb_path)
    elif antigen_chains:
...
def write_pdbs(pdb_hits: dict, output_path : str) -> None:
...
def main(input_pdb_path: str, output_path: str, binder_seq : str = None, antigen_seq : str = None, binder_chains : str = None, anitgen_chains : str = None, order : str = None):


    pdb_to_write = seq_and_chain_search(input_pdb_path,binder_seq,antigen_seq,binder_chains,anitgen_chains)
    write_pdbs(pdb_to_write,output_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input_path", type=str, help="path of the input pdb", required=True)
    parser.add_argument("-o", "--output_path", type=str, help="path of the output pdb", required=True)
