...
VALID_3_LINE_STARTERS = {"ALA": "A", "CYS": "C", "ASP": "D", "GLU": "E", "GLY": "G", "HIS": "H", "ILE": "I", "LYS": "K",
                         "LEU": "L", "MET": "M",
                         "ASN": "N", "PHE": "F", "PRO": "P", "GLN": "Q", "ARG": "R", "SER": "S", "THR": "T", "VAL": "V",
                         "TRP": "W", "TYR": "T"}
REF_SEQ = ""
PROTEIN_OUT_FILES = {}
...
def parse_full_pdb(input_pdb_path: str) -> list:
    ...
    with open(input_pdb_path, "r") as inputPDB:
        for line in inputPDB:
            if "ATOM" in line[0:6] or "TER" in line[0:6] or "END" in line[0:6] or "HETATM" in line[
                                                                                              0:6] and not "#" in line:
...
def mesh_search(sm_3_names: str, chains: str, seqs: str, input_pdb_path: str = None, search_radius: float = 8.0,
                seq_id: int = 95, take_first_only: bool = True, indexes: list = None) -> list:
    mesh_atoms = []
    mesh_atoms_filtered = []
    ...
    if sm_3_names:
        mesh_atoms += sm_search(sm_names=sm_3_names, take_first_ligand_only=take_first_only,
                                input_pdb_path=input_pdb_path, res_indexs=indexes)
...
def seq_search(seq: str, parsed_pdb: list = None, input_pdb_path: str = None, seq_identity: int = 95,
               search_first: bool = True) -> list:
    """
...
def seq_and_chain_search(input_pdb_path: str, binder_seq: str = None, target_protein_seq: str = None,
                         binder_chains: str = None, target_protein_chains: str = None, order: str = "chains",
                         seq_id: int = 95, sm_name: str = None, target_sm_chains: str = None, search_sm: str = "chains",
                         take_first_ligand_only: bool = True, mesh_search_results: list = [], res_indexs: str = None,
                         do_ref: str = None) -> [dict, int]:
    """
    ...
    if (target_protein_seq and target_protein_chains):
        if order == "seq":
            if mesh_search_results:
                pdb_target_protein_seq_hits = seq_search(target_protein_seq, mesh_search_results, None,
                                                         seq_identity=seq_id)
            else:
            ...
            pdb_hits["target_protein"] = seq_search(target_protein_seq, pdb_target_protein_chain_hits, None,
                                                    seq_identity=seq_id)
    ...
    if target_sm_chains or sm_name:
        ...
        if mesh_search_results:
            pdb_hits["sm"] = sm_search(sm_name, target_sm_chains, search_sm, take_first_ligand_only, input_pdb_path,
                                       mesh_search_results, res_indexs=res_indexs)
        else:
            pdb_hits["sm"] = sm_search(sm_name, target_sm_chains, search_sm, take_first_ligand_only, input_pdb_path,
                                       res_indexs=res_indexs)

    if ((target_protein_seq or target_protein_chains) and do_ref == "1v1"):
        global REF_SEQ
    ...
    elif ((target_protein_seq or target_protein_chains) and do_ref == "MSA"):
        target_seq = get_sequence(pdb_hits["target_protein"])
        return [pdb_hits, target_seq]
...
def write_pdbs(pdb_name: str, pdb_hits: dict, output_path: str, name_conversion: dict = None, binder_chain_rename="C",
    ...
    with open(output_path + "/" + pdb_name, "w") as output_pdb:
        ...
        for key in pdb_hits:
            ...
            if key == "target_protein" and new_numbering_list != None:
                out_res_index = 0
    ...
    for line1 in sm_pdb_lines:
        ...
        if atom_name == atom_name_o:
            ...
            for line2 in sm_pdb_lines:
                ...
                distence = math.sqrt(
                    math.pow(atom1_coords[0] - atom2_coords[0], 2) + math.pow(atom1_coords[1] - atom2_coords[1],
                                                                              2) + math.pow(
...
def main(input_pdb_path: str, output_path: str, binder_seq: str = None, target_protein_seq: str = None,
         binder_chains: str = None, anitgen_chains: str = None, order: str = None, seq_identity: int = 95,
         sm_name: str = None, target_sm_chains: str = None, search_sm: str = "chains", take_first_ligand: bool = True,
         name_conversion: dict = None, target_rename: str = "A", sm_rename: str = "B", binder_rename: str = "C",
         mesh_radius: float = 8.0, mesh_search_vars: str = ";;;", res_indexes: str = None,
         do_ref: str = None) -> None:
    """
    ...
    if len(mesh_search_vars) > 2:
        ...
        mesh_results = mesh_search(sm, chains, seqs, input_pdb_path, mesh_radius, seq_identity, take_first_ligand,
                                   indexes)
        pdb_to_write = seq_and_chain_search(input_pdb_path, binder_seq, target_protein_seq, binder_chains,
                                            anitgen_chains, order, seq_identity, sm_name, target_sm_chains, search_sm,
                                            take_first_ligand, mesh_results, res_indexs=res_indexes, do_ref=do_ref)
    else:
        pdb_to_write = seq_and_chain_search(input_pdb_path, binder_seq, target_protein_seq, binder_chains,
                                            anitgen_chains, order, seq_identity, sm_name, target_sm_chains, search_sm,
                                            take_first_ligand, res_indexs=res_indexes, do_ref=do_ref)
...
if __name__ == "__main__":
    ...
    if args.sm_ligand_reference_path:
        ...
        if args.name_sm_atoms_same:
            first_ligand = sm_search(sm_name, target_sm_chains, args.search_first_sm, True,
                                     args.sm_ligand_reference_path, res_indexs=arg_spliter(args.target_sm_index))
    ...
    if os.path.isfile(args.input_path):
        if args.seq_target_ref_pdb or args.seq_target_align:
            ...
            if args.name_sm_atoms_same:
                ...
                if args.target_sm_index and (args.target_sm_chains or args.target_sm_3_names):
                    first_ligand = sm_search(sm_name, target_sm_chains, args.search_first_sm, True, args.input_path,
                                             res_indexs=arg_spliter(args.target_sm_index))
                else:
                    first_ligand = sm_search(sm_name, target_sm_chains, args.search_first_sm, True, args.input_path,
                                             res_indexs=arg_spliter(args.target_sm_index))
                ...
                main(args.input_path, args.output_path, args.binder_seqs, args.target_protein_seqs, args.binder_chains,
                     args.target_protein_chains, args.search_first_protein, args.seq_identity, args.target_sm_3_names,
                     args.target_sm_chains, args.search_first_sm, args.take_first_sm_only, name_conversion_key,
                ...
                if args.name_sm_atoms_same and file_counter == 1 and not args.sm_ligand_reference_path:
                    if args.target_sm_index and (args.target_sm_chains or args.target_sm_3_names):
                        ...
                        first_ligand = sm_search(sm_name, target_sm_chains, args.search_first_sm, True,
                                                 ...
                                                 res_indexs=res_indexs)
                    ref_tree = get_sm_atom_names_and_connections(first_ligand, args.bond_length)
                    if args.seq_target_ref_pdb and args.seq_target_ref_pdb.split("/")[
                        -1] != file and args.seq_target_align:
                    ...
                    main(os.path.join(args.input_path, file), args.output_path, args.binder_seqs,
                         args.target_protein_seqs, args.binder_chains, args.target_protein_chains,
                         args.search_first_protein, args.seq_identity, args.target_sm_3_names, args.target_sm_chains,
                         args.search_first_sm, args.take_first_sm_only, {}, args.target_protein_chain_rename,
                         args.target_sm_chain_rename, args.binder_chain_rename, args.search_radius, args.mesh_search,
                    ...
                    main(os.path.join(args.input_path, file), args.output_path, args.binder_seqs,
                         args.target_protein_seqs, args.binder_chains, args.target_protein_chains,
                         args.search_first_protein, args.seq_identity, args.target_sm_3_names, args.target_sm_chains,
                         args.search_first_sm, args.take_first_sm_only, name_conversion_key,
                         args.target_protein_chain_rename, args.target_sm_chain_rename, args.binder_chain_rename,
                         args.search_radius, args.mesh_search, res_indexes=args.target_sm_index,
                         do_ref=args.seq_target_align)
